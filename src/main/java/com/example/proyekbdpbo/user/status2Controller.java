...
public class status2Controller {
    ...
    private void loadEstimationTime() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                String estimasi = rs.getString("estimasi_sampai");
                estimationTime.setText(estimasi != null ? estimasi : "Not available yet");
            } else {
                estimationTime.setText("Not found");
            }

        } catch (SQLException e) {
            estimationTime.setText("Fail to get estimation time");
            e.printStackTrace();
        }
    }
